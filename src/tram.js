  ...
  var doc = document
    , win = window
    , store = {}
    , testElement = doc.createElement('a').style
  ...
  var testFeature = function (prop) {
    // unprefixed case
    if (prop in testElement) return { dom: prop, css: prop };
    // test all prefixes
    var i, domProp, domSuffix = prop.charAt(0).toUpperCase() + prop.slice(1);
    for (i = 0; i < domPrefixes.length; i++) {
      domProp = domPrefixes[i] + domSuffix;
      if (domProp in testElement) return { dom: domProp, css: cssPrefixes[i] + prop };
    }
  };
